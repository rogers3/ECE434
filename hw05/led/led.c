...
static unsigned int gpioRedLED = 44;           ///< Default GPIO for the LED is 60
module_param(gpioRedLED, uint, S_IRUGO);       ///< Param desc. S_IRUGO can be read/not changed
MODULE_PARM_DESC(gpioRedLED, " GPIO LED number (default=60)");     ///< parameter description
static unsigned int gpioBlueLED = 45;           ///< Default GPIO for the LED is 60
module_param(gpioBlueLED, uint, S_IRUGO);       ///< Param desc. S_IRUGO can be read/not changed
MODULE_PARM_DESC(gpioBlueLED, " GPIO LED number (default=60)");     ///< parameter description
...
static bool ledBlueOn = 0;                      ///< Is the LED on or off? Used for flashing
...
static int flash(void *arg){
   printk(KERN_INFO "EBB LED: Thread has started running %p\n", arg);
   while(!kthread_should_stop()){           // Returns true when kthread_stop() is called
      set_current_state(TASK_RUNNING);
      if (mode==FLASH) ledOn = !ledOn;      // Invert the LED state
      else if (mode==ON) ledOn = true;
      else ledOn = false;
      gpio_set_value(gpioRedLED, ledOn);       // Use the LED state to light/turn off the LED
      set_current_state(TASK_INTERRUPTIBLE);
      msleep(blinkPeriod/2);                // millisecond sleep for half of the period
      countBlueLed = countBlueLed + 1;
      if(countBlueLed >= 3){
         ledBlueOn  =! ledBlueOn;
         gpio_set_value(gpioBlueLED, ledBlueOn);
         countBlueLed = 0;
      }
   }
...
static int __init ebb_LED_init(void){
   ...
   sprintf(ledName, "led%d", gpioRedLED);      // Create the gpio name for /sys/ebb/led60
   sprintf(ledName, "led%d", gpioBlueLED);      // Create the gpio name for /sys/ebb/led60
   ...
   ledOn = true;
   gpio_request(gpioRedLED, "sysfs");          // gpioLED is 60 by default, request it
   gpio_direction_output(gpioRedLED, ledOn);   // Set the gpio to be in output mode and turn on
   gpio_export(gpioRedLED, false);  // causes gpio60 to appear in /sys/class/gpio
                                 // the second argument prevents the direction from being changed
   gpio_request(gpioBlueLED, "sysfs");          // gpioLED is 60 by default, request it
   gpio_direction_output(gpioBlueLED, ledOn);   // Set the gpio to be in output mode and turn on
   gpio_export(gpioBlueLED, false);  // causes gpio60 to appear in /sys/class/gpio
                                 // the second argument prevents the direction from being changed
...
static void __exit ebb_LED_exit(void){
   ...
   kobject_put(ebb_kobj);                   // clean up -- remove the kobject sysfs entry
   gpio_set_value(gpioRedLED, 0);              // Turn the LED off, indicates device was unloaded
   gpio_unexport(gpioRedLED);                  // Unexport the Button GPIO
   gpio_free(gpioRedLED);                      // Free the LED GPIO
   gpio_set_value(gpioBlueLED, 0);              // Turn the LED off, indicates device was unloaded
   gpio_unexport(gpioBlueLED);                  // Unexport the Button GPIO
   gpio_free(gpioBlueLED);                      // Free the LED GPIO
